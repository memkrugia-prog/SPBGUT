...
const CHANGELOG = [
 { v:'0.2.0', date:'сегодня', notes:[
   ...
   'Вход через Telegram (Login Widget), сессии — не разлогинивает',
   'Все пользователи входят под своим Telegram-логином (включая админа)',
   'Рассылка в группу через бота, напоминания о диктанте',
   'Турнир день/неделя/месяц/год, статистика, стрик и штрафы' ]},
 { v:'0.1.0', date:'ранее', notes:['Первый релиз: словарь, диктанты, локальные профили'] }
];
...
async function addActivity(userId,{points=0,answered=0,correct=0,dictations=0}){
  await db.q(`INSERT INTO activity(user_id,day,points,answered,correct,dictations)
...
app.post('/api/broadcast', auth, adminOnly, async (req,res)=>{
  ...
  if(!chatId) return res.status(400).json({error:'Сначала отправь /setbroadcast в группе'});
...
app.post('/telegram/webhook', async (req,res)=>{
  ...
  if(text.startsWith('/setbroadcast')){
    ...
    return tg.sendMessage(chatId,'✅ Эта группа теперь получает рассылку.');
  }
  ...
  if(text.startsWith('/help')) return tg.sendMessage(chatId,'Команды: /start, /setbroadcast (админ), /broadcast <текст> (админ)');
});
